...
namespace HoDBlocks.Winch
{
    public class BlockEntityWinch : BlockEntityOpenableContainer
    {
        ...
        public override void Initialize(ICoreAPI api)
        {
            ...
            config = ModConfig.ReadConfig<Config.Config>(api, "HoDBlocksConfig.json") ?? new Config.Config();
            if (inventory == null)
            {
        ...
        private MeshData GenMesh(string subPart)
        {
            if (Api is ICoreClientAPI capi)
            {
                ...
                string shapePath = "hodblocks:shapes/block/winch/" + subPart + ".json";
        ...
        private MeshData GenRopeMesh()
        {
            ...
            {
                Block block = Api.World.BlockAccessor.GetBlock(Pos);
                string shapePath = "hodblocks:shapes/block/winch/rope1x1.json";
                Shape shape = Shape.TryGet(Api, shapePath);
                if (shape == null) return null;
        ...
        private MeshData GenKnotMesh()
        {
            ...
            {
                Block block = Api.World.BlockAccessor.GetBlock(Pos);
                string shapePath = "hodblocks:shapes/block/winch/knot.json";
                Shape shape = Shape.TryGet(Api, shapePath);
                if (shape == null) return null;
        ...
        {
            float lowerSpeed = HoDBlocksModSystem.LoadedConfig.WinchLowerSpeed;
            float raiseSpeed = HoDBlocksModSystem.LoadedConfig.WinchRaiseSpeed;
        ...
        {
            get { return Lang.Get("hodblocks:Winch"); }
        }
    }
}